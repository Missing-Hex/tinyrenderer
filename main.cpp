...
void triangle(int ax, int ay, int bx, int by, int cx, int cy, TGAImage &framebuffer, TGAColor ca, TGAColor cb, TGAColor cc) {
...
#pragma omp parallel for
    for (int x=bbminx; x<=bbmaxx; x++) {
        for (int y=bbminy; y<=bbmaxy; y++) {
            ...
            TGAColor color;
            for(int c = 0; c < 3; c++)
                color[c] = static_cast<unsigned char>(alpha * ca[c] + beta * cb[c] + gamma * cc[c]);
            framebuffer.set(x, y, color);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    */
    ...
    triangle(ax, ay, bx, by, cx, cy, framebuffer, red, green, blue);
